...
<div class="col-12 col-md-9 px-md-0 py-5">
    ...
    <div class="row px-xl-5 mx-0 mx-md-5 pt-3">
        <div class="col-12 px-0 mb-4">
            <a href="{{route('mark.all.read')}}" class="float-left font-eb font-16 color-r px-4 py-1 border-1 br-7 noti-btn" data-toggle="tooltip" data-placement="top" title="Mark all new notifications as read.">Mark all as read</a>
            <a href="{{route('all.read.notifications')}}" class="float-right font-eb font-16 color-r bg-transparent px-4 py-1 border-1 br-7 noti-btn" data-toggle="tooltip" data-placement="top" title="View old or read notifications.">Old notifications</a>
        </div>
        <div class="col-12 bg-lgray br-7 border-0 box-shadow mb-4 p-3">
            @if($user->unreadNotifications->count() > 0)
                @foreach($user->unreadNotifications as $not)
                    ...
                    <div class="py-2 py-md-0 text-left clearfix">
                        <div class="float-left noti-left">
                            ...
                            <div class="d-inline-block pt-1 pl-0 pl-md-2">
                                <p class="font-eb mt-2 font-16 color-b noti-text"><a href="{{route('visit.profile',$follower->slug)}}" class="color-r">{{$follower->name}}</a> Followed You </p>
                            </div>
                        </div>
                    ...
                    <div class="py-2 py-md-0 text-left clearfix">
                        <div class="float-left noti-left">
                            ...
                            <div class="d-inline-block pt-1 pl-0 pl-md-2">
                                <p class="font-eb mt-2 font-16 color-b noti-text"><a href="{{route('visit.profile',$likedBy->slug)}}" class="color-r">{{$likedBy->name}}</a> Liked your <a href="{{route('rc.view',$rc->id)}}" class="color-r">risk control</a>  </p>
                            </div>
                        </div>
                        <span class="float-right noti-right">
                ...
                @endforeach
            @else
            <p class="lead">No new notification to show. You can check old notifications by clicking old notifications. </p>
            @endif
