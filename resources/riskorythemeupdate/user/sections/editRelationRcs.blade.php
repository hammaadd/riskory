...
<div class="row p-1 p-md-4">
    ...
    <input type="checkbox" name="selected[]" id="check{{$rcb->id}}" value="{{$rcb->id}}" data-title="{{$rcb->title}}" onclick="listRelation(this.id)"/>
        <label for="check{{$rcb->id}}"></label>
     </div>
     <div class="col-10 col-sm-11 bg-lgray br-20 border-0 box-shadow pl-0 pr-1 pr-md-4">
         <div class="row ml-0">
             <p class="br-tl-20 bg-white font-16 font-eb color-r px-3 py-1">{{views($rcb)->count()}} views</p>
             <div class="pt-1 pl-3">
         ...
         <div class="row px-md-4 px-1">
             <div class="col-12 col-md-5">
                 <p class="font-eb font-18 color-b mb-0">{{$rcb->title}}</p>
                 <p class="font-e font-14 text-underl"><a href="{{route('rc.view',$rcb->slug)}}" class="color-r" target="_blank">Click for more Info</a></p>
             </div>
             <div class="col-12 col-md-7">
                 <div class="modal-icon">
                    ...
                    <div class="d-inline-block">
                        ...
                        @endif
                                <p class="p-style font-eb text-center">{{$rcb->likes->count()}}</p>
                                
                            </div>
                            <div class="d-inline-block">
                        @if (!($rcb->dislikedBy(auth()->user())))
                        ...
                        @endif
                                <p class="p-style font-eb text-center">{{$rcb->dislikes->count()}}</p>
                                
                                
                            </div>
                            <div class="d-inline-block">
                            
                        @if (!($rcb->bookmarkedBy(auth()->user())))
                        ...
                        @endif
                                <p class="p-style font-eb text-center">{{$rcb->bookmarks->count()}}</p>
                            </div>
                </div>
             </div>
         </div>
