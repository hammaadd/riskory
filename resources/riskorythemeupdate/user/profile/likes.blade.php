...
@foreach($likes as $l)
<?php $rc = $l->rcs;?>
    <div class="col-12 bg-lgray br-7 border-0 box-shadow mt-4 mb-4">
        <div class="row">
            <div class="col-12 px-0">
                <span class="br-tl-7 bg-white font-16 font-eb color-r px-3 py-1">{{views($rc)->count()}} views</span>
                
                <span class="br-tr-7 bg-white font-16 font-e color-r px-3 py-1 float-right">
                    @if($rc->status == 'U')
                        Under Discussion
                    @elseif($rc->status == 'P')
                        Pending
