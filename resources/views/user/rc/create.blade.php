                        ...
                        <fieldset id="firstfieldset" class="form-group">
                            ...
                            <div class="mb-4">
                                ...
                                <option value="{{$con->id}}" @if(old('industry')) @if(in_array($con->id,old('industry'))) Selected @endif @endif><strong>{{$con->name}}</strong> </option>
                                    @if($con->children)
                                    @foreach($con->children as $lvl1)
                                        <option value="{{$lvl1->id}}" @if(old('industry')) @if(in_array($lvl1->id,old('industry'))) Selected @endif @endif>&nbsp;&nbsp;-&nbsp;{{$lvl1->name}} </option>
                                        @if($lvl1->children)
                                        @foreach($lvl1->children as $lvl2)
                                            <option value="{{$lvl2->id}}" @if(old('industry')) @if(in_array($lvl2->id,old('industry'))) Selected @endif @endif>&nbsp;&nbsp;&nbsp;&nbsp;--&nbsp;{{$lvl2->name}} </option>
                                            @if($lvl2->children)
                            ...
                            <div class="mb-4">
                                ...
                                <select class="js-example-basic-multiple form-control p-5 br-20 box-shadow border-0 font-16 font-e color-dg" name="process[]" multiple="multiple">
                                    ...
                                    <option value="{{$con->id}}" @if(old('process')) @if(in_array($con->id,old('process'))) Selected @endif @endif><strong>{{$con->name}}</strong> </option>
                                        @if($con->children)
                                        @foreach($con->children as $lvl1)
                                            <option value="{{$lvl1->id}}" @if(old('process')) @if(in_array($lvl1->id,old('process'))) Selected @endif @endif>&nbsp;&nbsp;-&nbsp;{{$lvl1->name}} </option>
                                            @if($lvl1->children)
                                            @foreach($lvl1->children as $lvl2)
                                                <option value="{{$lvl2->id}}" @if(old('process')) @if(in_array($lvl2->id,old('process'))) Selected @endif @endif>&nbsp;&nbsp;&nbsp;&nbsp;--&nbsp;{{$lvl2->name}} </option>
                                                @if($lvl2->children)
                                                @foreach($lvl2->children as $lvl3)
                            ...
                            <div class="mb-4">
                                ...
                                <select class="js-example-basic-multiple form-control p-5 br-20 box-shadow border-0 font-16 font-e color-dg" name="framework[]" multiple="multiple">
                                    ...
                                    <option value="{{$con->id}}" @if(old('framework')) @if(in_array($con->id,old('framework'))) Selected @endif @endif><strong>{{$con->name}}</strong> </option>
                                        @if($con->children)
                                        @foreach($con->children as $lvl1)
                                            <option value="{{$lvl1->id}}" @if(old('framework')) @if(in_array($lvl1->id,old('framework'))) Selected @endif @endif>&nbsp;&nbsp;-&nbsp;{{$lvl1->name}} </option>
                                            @if($lvl1->children)
                                            @foreach($lvl1->children as $lvl2)
                                                <option value="{{$lvl2->id}}" @if(old('framework')) @if(in_array($lvl2->id,old('framework'))) Selected @endif @endif>&nbsp;&nbsp;&nbsp;&nbsp;--&nbsp;{{$lvl2->name}} </option>
                                                @if($lvl2->children)
                                                @foreach($lvl2->children as $lvl3)
