...
<div class="px-0 col-12 col-md-9 py-2">
    <div class="row pt-4 mx-0">
        <div class="pl-0 col-sm-auto col-12">
            <div class="">
            <p class="bg-lblue font-eb font-18 py-2 px-5 rounded-right-xl"><i><img src="{{asset('assets/images/Mask Group 10@2x.png')}}" class="align-bottom" width="35px"></i>&nbsp;&nbsp;
                @if(isset($control))
                    {{$control->name}}
                @endif
                @if(isset($tag))
    ...
    @if(isset($control))
        @if($data->count()<1)
            <h6 class="lead">No results found for "{{$control->name}}" </h6>
        @endif
    @endif
    @if(isset($tag))
        @if($data->count()<1)
            <h6 class="lead">No results found for "{{$tag->name}}" </h6>
        @endif
    @endif
     {{-- Risk control starts here --}}
